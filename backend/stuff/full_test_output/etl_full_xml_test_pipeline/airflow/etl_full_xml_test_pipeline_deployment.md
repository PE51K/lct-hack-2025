# Deployment Guide for Full XML Test Pipeline
...
- `etl_full_xml_test_pipeline.py` - Основной DAG файл
- `etl_full_xml_test_pipeline_functions.py` - Модуль функций задач
- `etl_full_xml_test_pipeline_config.py` - Конфигурационный файл
...
```bash
cp etl_full_xml_test_pipeline.py $AIRFLOW_HOME/dags/
cp etl_full_xml_test_pipeline_functions.py $AIRFLOW_HOME/dags/
cp etl_full_xml_test_pipeline_config.py $AIRFLOW_HOME/dags/
```
...
#### Источник данных
- **Connection ID:** `etl_full_xml_test_pipeline_source`
- **Connection Type:** `folder`
- **Host/URI:** `file://d:/lct-hack-2025/backend/parsers/sample/XML/`

#### Целевое хранилище  
- **Connection ID:** `etl_full_xml_test_pipeline_target`
...
```sql
-- Создайте таблицу public.Full XML Test Pipeline_data
-- в вашей целевой системе postgres

CREATE TABLE public.Full XML Test Pipeline_data (
...
airflow variables set etl_full_xml_test_pipeline_batch_size 1000
airflow variables set etl_full_xml_test_pipeline_parallel_workers 2
airflow variables set etl_full_xml_test_pipeline_notification_email "test_user@company.com"
```
...
airflow dags list | grep etl_full_xml_test_pipeline

# Тестовый запуск
airflow dags test etl_full_xml_test_pipeline $(date -d "yesterday" +%Y-%m-%d)

# Проверка задач
airflow tasks list etl_full_xml_test_pipeline
```
...
airflow pools set etl_full_xml_test_pipeline_pool 2 "Pool for etl_full_xml_test_pipeline"
```
...
### Логи:
- Логи Airflow: `$AIRFLOW_HOME/logs/dags/etl_full_xml_test_pipeline/`
- Логи приложения: `/var/log/airflow/dags/etl_full_xml_test_pipeline/`
