...
from dag_generation import ETLDAGGenerationSystem, __version__, print_system_info
    ...
    try:
        ...
        if result.get("success", True):
            ...
            if "dag_file" in generated_files:
                ...
                with open(dag_file_path, encoding="utf-8") as f:
            ...
            print(
                f"   • Тип источника: {pipeline_config.pipeline_config.extract_config.source_type}"
            )
            print(
                f"   • Формат данных: {pipeline_config.pipeline_config.extract_config.content_type}"
            )
            print(
                f"   • Размер данных: {pipeline_config.pipeline_config.extract_config.source_metrics.estimated_size_mb} МБ"
            )
            print(
                f"   • Количество записей: {pipeline_config.pipeline_config.extract_config.source_metrics.estimated_records:,}"
            )
            print(
                f"   • Целевое хранилище: {pipeline_config.pipeline_config.load_config.target_storage_type}"
            )
            print(
                f"   • Ожидаемое время: {pipeline_config.pipeline_config.estimated_runtime_minutes} мин"
            )
...
async def demo_multiple_sources():
    ...
    sources = [
        ...
        {"url": "postgres://localhost:5432/demo_db", "name": "PostgreSQL Demo Data"},
    ]
