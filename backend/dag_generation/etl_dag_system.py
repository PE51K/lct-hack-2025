...
class ETLDAGGenerationSystem:
    ...
    def __init__(self, output_base_dir: str = "generated_etl_pipelines"):
        ...
        self.creation_report = {"created_pipelines": [], "total_pipelines": 0, "last_created": None}
        ...
        **kwargs,
        ...
        try:
            ...
            pipeline_config = await self.dag_builder.build_complete_pipeline_from_url(
                source_url=source_url, pipeline_name=pipeline_name, owner=owner, team=team
            )
            ...
            print(
                f"   🏪 Хранилище: {pipeline_config.pipeline_config.load_config.target_storage_type}"
            )
            ...
            config_saved = await self.dag_builder.save_pipeline_config(
                pipeline_config, str(pipeline_dir)
            )
            ...
            if generate_airflow_code:
                ...
                print("✅ Airflow DAG сгенерирован!")
            ...
            print(
                f"📊 Оценка времени работы: {pipeline_config.pipeline_config.estimated_runtime_minutes} мин"
            )
            print(f"💾 Конфигурация: {Path(config_saved).name}")

            if generate_airflow_code:
                print("🚀 Готов к развертыванию в Airflow!")
    ...
    async def create_multiple_pipelines(
        self, sources: list[dict[str, str]], **kwargs
    ) -> list[dict[str, Any]]:
        """
        ...
        for i, source in enumerate(sources, 1):
            ...
            try:
                result = await self.create_complete_etl_pipeline(
                    source_url=source["url"], pipeline_name=source["name"], **kwargs
                )
    ...
    async def _create_pipeline_report(
        self, pipeline_config: CompletePipelineWithAI, creation_result: dict[str, Any]
    ) -> str:
        ...
        if pipeline_config.ai_recommendations:
            report_lines.extend(["## 🤖 AI Рекомендации", ""])
        ...
        # Созданные файлы
        report_lines.extend(["## 📁 Созданные файлы", ""])
    ...
    async def get_system_status(self) -> dict[str, Any]:
        ...
        successful_pipelines = len(
            [p for p in self.creation_report["created_pipelines"] if p.get("success", True)]
        )
...
async def example_multiple_pipelines():
    ...
    sources = [
        ...
        {"url": "postgres://localhost:5432/source_db", "name": "PostgreSQL Extract Pipeline"},
    ]

    results = await system.create_multiple_pipelines(
        sources=sources, owner="data_engineering_team", team="analytics"
    )
